...
peak_days |>
  ...
  datatable(caption = "Top 15 Historical Peak Days")
```
...
tidy(ets_model) |>
  datatable(caption = "ETS Model Summary")
...
predicted_peaks_updated |>
  select(peak_rank, date_2025, month_day,
         ...
         data_status) |>
  mutate_if(is.numeric, round, 0) |>
  datatable(caption = "Top 5 Predicted Peak Days for 2025")
```
...
datatable(true_accuracy_metrics)
```
...
## How to Use This Daily Forecast

1. **Run this report daily** - The analysis automatically pulls the latest data
2. **Check 2025 performance** - See if the season is running high/low vs historical
3. **Review peak predictions** - Top 5 predicted peak days with dates
4. **Monitor adjustments** - Watch how predictions change as 2025 data comes in

## Key Outputs

- **Predicted peak dates** for 2025 fishing season
- **Confidence intervals** around predictions
- **Performance tracking** of 2025 vs historical patterns
- **Daily updates** that improve accuracy over time

## Interpretation Guide

- **Green line**: Actual 2025 observations (real data)
- **Blue line**: Historical average pattern
- **Purple line**: Adjusted predictions based on 2025 performance
- **Red dots**: Predicted peak days
...
- **ETS Model**: Exponential Smoothing State Space model for seasonal patterns
- **Adaptive System**: Predictions adjust based on 2025 vs historical performance ratio
- **Confidence Intervals**: Based on model residual standard deviation
- **Update Frequency**: Daily, as new data becomes available

## Data Sources

- **Primary**: Alaska Department of Fish & Game live fish count API
- **Location**: Kenai River counting station (ID: 40)
- **Species**: Sockeye salmon (ID: 420)
- **Years**: 2019-2025

## Limitations

- Predictions assume 2025 follows similar seasonal patterns to historical years
- Weather, environmental conditions, and other factors may cause deviations
- Model accuracy improves as more 2025 data becomes available
